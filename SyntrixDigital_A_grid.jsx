        ...
        {sent ? (
          ...
          <div style={{padding:"32px 32px 28px"}}>
            ...
            {step===4 && <>
              ...
              <p style={{fontSize:11,color:"#cbd5e1",textAlign:"center",marginTop:10}}>Wir melden uns nur, wenn wir dir wirklich helfen können.</p>
            </>}
          </div>
        )}
      </div>
...
function calcResult(ans){
  ...
  if(pct<35){status="🔴 Unstrukturiert";statusColor="#ef4444";opp="Aufbau eines systematischen Funnels";next="Potenzialanalyse buchen & Funnel-Konzept entwickeln";}
  else if(pct<65){status="🟡 Im Aufbau";statusColor="#f59e0b";opp="Optimierung & Automatisierung bestehender Prozesse";next="System-Audit & gezielte Optimierungsmaßnahmen";}
  else{status="🟢 Skalierbar";statusColor="#22c55e";opp="Skalierung & Effizienzsteigerung";next="Performance-Analyse & Skalierungsstrategie";}
...
function PotenzialFunnel({ onBack, onCalendly }) {
  ...
  const goBook=()=>{
    const p=new URLSearchParams({name:lead.name,email:lead.email,a1:`Score:${result?.score}/100|${result?.status}`,a2:`Problem:${result?.problem}`,a3:result?.opportunity});
    onCalendly({...lead,...result,params:p.toString()});
  };
  return (
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#070c18,#0f172a)",padding:"48px 20px",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:740,margin:"0 auto"}}>
        ...
        <div style={{background:"rgba(255,255,255,0.02)",borderRadius:20,border:"1px solid rgba(255,255,255,0.08)",overflow:"hidden",minHeight:650}}>
          <div className="calendly-inline-widget" data-url={`https://calendly.com/kontakt-syntrixdigital/30min?hide_gdpr_banner=1&primary_color=0ea5e9${leadData?.params?"&"+leadData.params:""}`} style={{minWidth:320,height:650}}/>
        </div>
